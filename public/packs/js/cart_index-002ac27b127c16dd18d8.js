...
/* WEBPACK VAR INJECTION */(function($) {//To Close The UpdateServiceModal
$(".btnClose").on("click", function () {
  $("#update_service").hide();
}); //Update Product Quantity In Cart

update_quanatity = function update_quanatity(quantity, orderId, itemPrice) {
  var newPrice = quantity * itemPrice;
...
var defaultPrice;
var orderedDuration;
var totalAmount; //Show the PopUp to Change the Service Date Duration and Time

update_service_popup = function update_service_popup(order_id, date, default_duration, start_time, end_time, default_price, order_duration) {
  orderId = order_id;
  defaultPrice = default_price;
  orderedDuration = order_duration;
  defaultDuration = default_duration;
  startTime = start_time;
  endTime = end_time;
  totalAmount = defaultPrice / defaultDuration * orderedDuration;
  $("#update_service").show();
  $("#date_in_popup").val(date);
  $("#duration_in_popup").attr("step", defaultDuration);
  $("#duration_in_popup").val(orderedDuration);
  $.ajax({
...
$("#date_in_popup, #duration_in_popup").on("change", function () {
  $("#date_in_popup").blur();
  var duration = $("#duration_in_popup").val();
  startTime = $("start_time" + orderId).val();
  alert(startTime);
  endTime = $("end_time" + orderId).val();
  totalAmount = defaultPrice / defaultDuration * duration;
  $.ajax({
    url: "../orders/time_slot",
...
}); //update service after clicking SaveButton in PopUp

update_service = function update_service() {
  date = $("#date_in_popup").val();
  duration = $("#duration_in_popup").val();
  $.ajax({
    url: "update_service",
    data: "order_id=" + orderId + "&date=" + date + "&duration=" + duration + "&start_time=" + startTime + "&end_time=" + endTime + "&total_amount=" + totalAmount + "&default_duration=" + defaultDuration,
    type: "post",
    success: function success(response) {
      $("#cart_date" + orderId).html(date);
      $("#duration" + orderId).html(duration);
      $("#start_time" + orderId).html(startTime);
      $("#end_time" + orderId).html(endTime);
      $("#amount" + orderId).html(totalAmount);
      $("#update_service").hide();
    }
  });
...
setTime = function setTime(user_selected_start_time, user_selected_end_time) {
  startTime = user_selected_start_time;
  endTime = user_selected_end_time;
  $("#button_save").attr("class", "btn btn-primary");
}; //Focus on Date If A User Click On Start_Time or End_time


first_select_date = function first_select_date() {
  $("#date").focus();
};
...
/******/ });
//# sourceMappingURL=cart_index-002ac27b127c16dd18d8.js.map